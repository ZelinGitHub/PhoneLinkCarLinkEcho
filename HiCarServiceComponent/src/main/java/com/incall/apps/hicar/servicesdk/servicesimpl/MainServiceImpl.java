...
public class MainServiceImpl extends BaseHiCarListener implements IMainService {
    ...
    public Object callMethodSync(String methodName, Map params) throws CAServiceException {
        ...
        switch (methodName) {
            //更新车配置
            case Contants.Method.UPDATE_CARCONFIG:
                ...
                break;
            //开始投屏
            case Contants.Method.START_PROJECTION:
                //开始投屏
                object = hiCarManager.startProjection();
                //请求共享上网
                HiCarServiceManager.getInstance().sendCarData(Contants.HiCarCons.DATA_TYPE_SHARE_NET, CommonUtil.requestShareNet(0, 1));
                break;
            case Contants.Method.IS_CONNECTED_DEVICE:
                object = hiCarManager.isConnectedDevice();

                break;
            //暂停投屏
            case Contants.Method.PAUSE_PROJECTION:
                hiCarManager.pauseProjection();
                break;
            case Contants.Method.STOP_PROJECTION:
                hiCarManager.stopProjection();
                //投屏结束，请求停止共享上网
                HiCarServiceManager.getInstance().sendCarData(Contants.HiCarCons.DATA_TYPE_SHARE_NET, CommonUtil.requestShareNet(0, 0));
                break;
            //启动蓝牙设备扫描
            case Contants.Method.START_HICAR_ADV:
                Log.d(TAG, "START_HICAR_ADV---");
                hiCarManager.sendMsgDelayed(HiCarServiceManager.MSG_START_ADV, 0);
                break;
            //断开设备连接
            case Contants.Method.DIS_CONNECTED:
                //断开设备连接
                hiCarManager.disconnectDevice();
                break;
            case Contants.Method.STOP_HICAR_ADV:
                hiCarManager.sendMsgDelayed(HiCarServiceManager.MSG_STOP_ADV, 0);
                break;
            case Contants.Method.SET_ICON:
                try {
                    byte[] b = (byte[]) params.get("bytes");
                    hiCarManager.sendCarData(Contants.HiCarCons.DATA_TYPE_BRAND_ICON_DATA, b);
                } catch (Exception e) {
                    e.printStackTrace();
                }
                break;
            case Contants.Method.GET_PHONE_NAME:
                object = hiCarManager.getPhoneName();
                break;
            //得到连接码
            case Contants.Method.GET_PINCODE:
                //得到连接码
                object = hiCarManager.getPinCode();
                break;
            case Contants.Method.IS_PLAY_ANIM:
                object = hiCarManager.isPlayAnim();
                break;
            case Contants.Method.IS_BT_CONNECTED:
                object = btManager.isConnected();
                break;
            case Contants.Method.OPEN_BT:
                btManager.bluetoothOn();
                break;
            case Contants.Method.HICAR_TY:
                object = params.get("hicar_ty");
                if (!object.toString().isEmpty()) {
                    HiCarDataCollectManager.getInstance().startHiCar(Integer.parseInt(object.toString()));
                }
                break;
            case Contants.Method.DISCONNECT_BT:
                if (btManager == null) {
    ...
    public void onDeviceChange(String s, int i, int i1) {
        ...
        switch (i) {
            ...
            case HiCarConst.EVENT_DEVICE_CONNECT:
                //发送设备已连接的事件
                ServiceManager.getInstance().postEvent(Contants.Event.DEVICE_CONNECT, i);
                break;
            //设备断开连接
            case HiCarConst.EVENT_DEVICE_DISCONNECT:
                //发送断开连接的事件
                ServiceManager.getInstance().postEvent(Contants.Event.DEVICE_DISCONNECT, i);
                break;
            //设备投影连接
            case HiCarConst.EVENT_DEVICE_PROJECT_CONNECT:
                ServiceManager.getInstance().postEvent(Contants.Event.DEVICE_PROJECT_CONNECT, i);
                break;
            //设备投影断开连接
            //没有找到调用的地方
            case HiCarConst.EVENT_DEVICE_PROJECT_DISCONNECT:
                //发送事件
                ServiceManager.getInstance().postEvent(Contants.Event.DEVICE_PROJECT_DISCONNECT, i);
                break;
            default:
        }
    }
    ...
    public void onDeviceServiceChange(String s, int i) {
        ...
        switch (i) {
            ...
            case HiCarConst.EVENT_DEVICE_SERVICE_PAUSE:
                ServiceManager.getInstance().postEvent(Contants.Event.DEVICE_SERVICE_PAUSE, i);
                break;
            //设备服务恢复 恢复投屏 203
            case HiCarConst.EVENT_DEVICE_SERVICE_RESUME:
                ServiceManager.getInstance().postEvent(Contants.Event.DEVICE_SERVICE_RESUME, i);
                break;
            //设备服务开始 启动投屏 204
            case HiCarConst.EVENT_DEVICE_SERVICE_START:
                ...
                ServiceManager.getInstance().postEvent(Contants.Event.DEVICE_SERVICE_START, i);
                break;
            //设备服务停止
            case HiCarConst.EVENT_DEVICE_SERVICE_STOP:
                ServiceManager.getInstance().postEvent(Contants.Event.DEVICE_SERVICE_STOP, i);
                break;
            //播放中
            case HiCarConst.EVENT_DEVICE_DISPLAY_SERVICE_PLAYING:
                ServiceManager.getInstance().postEvent(Contants.Event.DEVICE_DISPLAY_SERVICE_PLAYING, i);
                break;
            //播放失败
            case HiCarConst.EVENT_DEVICE_DISPLAY_SERVICE_PLAY_FAILED:
                ServiceManager.getInstance().postEvent(Contants.Event.DEVICE_DISPLAY_SERVICE_PLAY_FAILED, i);
                break;
            default:
        }
    }
    ...
    public void onDataReceive(String s, int i, byte[] bytes) {
        ...
        //商标图标数据
        if (i == Contants.HiCarCons.DATA_TYPE_BRAND_ICON_DATA) {
            //发送事件
            ServiceManager.getInstance().postEvent(Contants.Event.BRAND_ICON_DATA_CHANGE, s);
        }
        //共享上网
        else if (i == Contants.HiCarCons.DATA_TYPE_SHARE_NET) {
            ServiceManager.getInstance().postEvent(Contants.Event.REQUEST_SHARE_NET, bytes);
        }
        //主动断开车机连接通知0
        else if (i == Contants.HiCarCons.DATA_TYPE_DISCONNECT_BY_USER) {
            HiCarServiceManager.getInstance().sendCarData(Contants.HiCarCons.DATA_TYPE_DISCONNECT_BY_USER, CommonUtil.isUserDisconnect(bytes));
        }
        //打开app
        else if (i == Contants.HiCarCons.DATA_TYPE_REQUEST_APP) {
            Log.d(TAG, "onDataReceive() call postEvent(Contants.Event.OPEN_APP)");
            ServiceManager.getInstance().postEvent(Contants.Event.OPEN_APP, bytes);
        }
    ...
    public void onPinCode(String s) {
        ...
        ServiceManager.getInstance().postEvent(Contants.Event.PIN_CODE_CHANGE, s);
    }
    ...
    public void onBinderDied() {
        ...
        //发送事件
        ServiceManager.getInstance().postEvent(Contants.Event.HICAR_BINDER_DIED, null);
    }
    ...
    public void onPinCodeFailed() {
        ...
        //发送事件
        ServiceManager.getInstance().postEvent(Contants.Event.PIN_CODE_FAILED, null);
    }
}
