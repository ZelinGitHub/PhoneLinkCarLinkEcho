...
public class LogUtil {
     ...
     * 应用默认TAG
     */
    public static final String APP_TAG = "1.0.0.z_WTPhoneLink";
    /**
...
//                else if (new File(sdCardPath + "/" + MyApplication.getContext().getPackageName() + ".log")
//                        .exists()) {
//
//                    productMode = false;
//                }
    ...
    public static void f(String message) {
        try {
            saveToSDCard(message, LogFileType.ErrorFile);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
        ...
        try {
            ...
            switch (level) {
                ...
                case Log.ERROR:
                    ...
                    break;
                default:
                    Log.e(tag, "不支持的Log级别" + messages);
            }
            // 判断独立文件输出是否开启
            if (logToFileEnable) {
                // 全级别存储到一般的日志文件
                saveToSDCard(messages, LogFileType.Ordinary);
                if (level >= Log.ERROR) {
                    // Error以上级别日志存储到Error日志文件
                    saveToSDCard(messages, LogFileType.ErrorFile);
                }
            }
        } catch (Throwable e) {
            LogUtil.e(e);
...
//            if (st.getClassName().equals(PerformanceLog.class.getName())) {
//                continue;
//            }
